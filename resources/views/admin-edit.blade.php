...
<body >
    ...
    <div class="sidebar" id="sidebar">
        ...
        <ul class="nav-links">
            ...
            <li 8><a href="{{ route('/admin/users') }}"><i class="fa-solid fa-user-tie"></i> Gère les utilisateurs</a></li>
            ...
            <li class="nav-liksthis"><i class="fa-solid fa-pen"></i> Modification</li>
        </ul>Modification
    ...
    <td colspan="3">
        @if(session('success'))
            <div style="padding:10px;border-radius:8px;background:#c6f6d5;color:#22543d;margin-bottom:12px;">
                {{ session('success') }}
            </div>
        @endif

        @if($errors->any())
            <div style="padding:10px;border-radius:8px;background:#fed7d7;color:#742a2a;margin-bottom:12px;">
        ...
        <form action="{{ route('/edit-user/update', $user->id) }}" method="POST" style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;">
            @csrf
            @method('PUT')

            <input type="text" name="nom" value="{{ old('nom', $user->nom) }}" placeholder="Nom"
                   style="padding:8px;border:1px solid #cbd5e0;border-radius:6px;min-width:180px;">

            <input type="text" name="prenom" value="{{ old('prenom', $user->prenom) }}" placeholder="Prénom"
                   style="padding:8px;border:1px solid #cbd5e0;border-radius:6px;min-width:180px;">

            <input type="email" name="email" value="{{ old('email', $user->email) }}" placeholder="Email"
                   style="padding:8px;border:1px solid #cbd5e0;border-radius:6px;min-width:240px;">

            <select name="roles" style="padding:8px;border:1px solid #cbd5e0;border-radius:6px;min-width:180px;">
                <option value="admin" {{ old('roles', $user->roles) === 'admin' ? 'selected' : '' }}>admin</option>
                <option value="responsable" {{ old('roles', $user->roles) === 'responsable' ? 'selected' : '' }}>responsable</option>
                <option value="utilisateur_interne" {{ old('roles', $user->roles) === 'utilisateur_interne' ? 'selected' : '' }}>utilisateur_interne</option>
            </select>

            <button type="submit" class="btn btn-approve">Enregistrer</button>
         </form>
        </td>
